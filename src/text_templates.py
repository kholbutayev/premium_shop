...
def get_order_confirmation_template(item_amount_dict, cart_price, phone_number=None, home_adress=None):
    text = "🛒 Buyurtma tarkibi:\n\n"
    for item, amount in item_amount_dict.items():
        text += f"{item.get_name()} x{amount} — {item.get_price() * amount} so‘m\n"
    text += f"\n💰 Umumiy summa: {cart_price} so‘m\n"

    if phone_number:
        text += f"📞 Telefon: {phone_number}\n"
    if home_adress:
        text += f"🏠 Manzil: {home_adress}\n"
